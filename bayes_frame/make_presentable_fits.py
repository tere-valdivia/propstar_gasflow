...
sig_mle[Ks[0] > Kcut] = mle1[2][Ks[0] > Kcut]
...
sig_mle_x2_good = np.hstack([mle2[2][npeaks_map == 2],
                             mle2[2][npeaks_map == 2]])
# the MLE sigma values where two components are
sig_mle_x1_good = mle1[2][npeaks_map == 1]
...
sig_mle_over_x1_good = mle1[2][np.isfinite(sig_dr1) & (npeaks_map == 1)]
sig_mle_over_x2_good = np.hstack([mle2[2][np.isfinite(sig_dr1) & (npeaks_map == 2)],
                                  mle2[2][np.isfinite(sig_dr1) & (npeaks_map == 2)]])
...
plt.text(0.847, 450, r"$\mathrm{Pearson's}~r ="+ "{0:.2f}$".format(pearsonr(sig_dr1_x1, sig_mle_x1)[0]))
plt.xlim(0, None)
plt.legend()
