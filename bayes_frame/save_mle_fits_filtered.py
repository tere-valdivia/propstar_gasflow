...
ngaussmapfilefiltered = 'nested-sampling/npeaks_cut5_noislands.fits'
npeaksfileQA = 'nested-sampling/npeaks_cut5_noislands_QA.fits'
velcubetotalfile = 'nested-sampling/vel_components_cube_filtered_QA.fits'

params1gfileQA = file_mle_x1[:-5]+'_filtered_QA.fits'
params2gfileQA = file_mle_x2[:-5]+'_filtered_QA.fits'
params3gfileQA = file_mle_x3[:-5]+'_filtered_QA.fits'

mlex1, mle1head = fits.getdata(file_mle_x1, header=True)
mlex2, mle2head = fits.getdata(file_mle_x2, header=True)
#only for HC3N
mlex3, mle3head = fits.getdata(file_mle_x3, header=True)
...
if not os.path.exists(ngaussmapfilefiltered) or overwrite_cubes:
    ...
    fits.writeto('nested-sampling/mask_5_s{}.fits'.format(minsize1g), Karr_clean, header=header_flatten(fits.getheader(file_Ks)), overwrite=True)
    #2 components
    ...
    fits.writeto('nested-sampling/mask_5_2ndcomp_s{}.fits'.format(minsize2g), Karr2_clean, header=header_flatten(fits.getheader(file_Ks)), overwrite=True)
    # 3 components
...
fits.writeto(file_mle_x1[:-5]+'_filtered.fits', mlex1, mle1head, overwrite=True)
fits.writeto(file_mle_x2[:-5]+'_filtered.fits', mlex2, mle2head, overwrite=True)
fits.writeto(file_mle_x3[:-5]+'_filtered.fits', mlex3, mle3head, overwrite=True)
...
if not os.path.exists(velcubetotalfile) or overwrite_cubes:
    paramstot = np.zeros((3, mle1head['NAXIS2'], mle1head['NAXIS1'])) * np.nan
    paramstothead = npeakshead.copy()
    paramstothead['NAXIS'] = 3
    paramstothead['WCSAXES'] = 2
    paramstothead['NAXIS3'] = 3
