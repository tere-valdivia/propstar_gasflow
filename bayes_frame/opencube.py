...
from config import (file_hc3n_10_9, file_rms_hc3n_10_9, cube_save_kwargs)

def make_cube(files=file_hc3n_10_9,
              rms_files=file_rms_hc3n_10_9):
    """
    ...
    if type(files) == np.ndarray or type(files)==list:
    # if files.size > 1:
        ...
        spc = pyspeckit.Cube(files)
        rms = fits.getdata(rms_files)
