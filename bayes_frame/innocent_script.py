...
with warnings.catch_warnings():
    ...
    from pyspecnest.gaussian import get_gauss_model
...
# we leave the name as nh3_model but it is a gaussian
nh3_model = get_gauss_model(sp, sp.error,
...
sanity_check = np.isfinite(
    nh3_model.log_likelihood([1.59, 7.8, 0.29] * npeaks,
...
if plot_fit and i_am_root():
    ...
    print(mle_pars)
    mle_parinfo = sp.specfit.fitter._make_parinfo(params=mle_pars[0], parerror=mle_pars[1], npeaks=npeaks)

    try:
        sp.specfit.plot_fit() # it should not even need inputs
    ...
    if npeaks > 1:
        plt.annotate('ln(Z{}/Z{}) = {:.2f}'.format(npeaks, 0,
                     ...
                     xycoords='axes fraction')
    # in python3, showing the plot cleans it from the figure
    # if show_fit:
    #     plt.show()
...
if plot_corner and i_am_root():
    ...
    c.add_chain(a.data[:, 2:], parameters=parameters)
    
    # c.add_chain(a.data[unfrozen_slice], parameters=parameters)
    
    c.configure(statistics="max", summary=True, cloud=True, sigmas=np.linspace(0, 2, 5), colors=['#49be25'])
